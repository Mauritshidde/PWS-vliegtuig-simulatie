            ...
            for (int k = 1; k < ny - 1; k++)
            {
                if (!mesh.at(i).at(j).at(k).boundary)
                {
                    double duDt = -(mesh.at(i).at(j).at(k).velocityX * (mesh.at(i).at(j).at(k).velocityX - mesh.at(i).at(j - 1).at(k).velocityX) / dx +
                                    ...
                                    mesh.at(i).at(j).at(k).velocityZ * (mesh.at(i).at(j).at(k).velocityX - mesh.at(i - 1).at(j).at(k).velocityX) / dz) /
                                  dx;
                    ...
                    double dvDt = -(mesh.at(i).at(j).at(k).velocityX * (mesh.at(i).at(j).at(k).velocityY - mesh.at(i).at(j - 1).at(k).velocityY) / dx +
                                    ...
                                    mesh.at(i).at(j).at(k).velocityZ * (mesh.at(i).at(j).at(k).velocityY - mesh.at(i - 1).at(j).at(k).velocityY) / dz) /
                                  dy;
                    ...
                    double dwDt = -(mesh.at(i).at(j).at(k).velocityX * (mesh.at(i).at(j).at(k).velocityZ - mesh.at(i).at(j - 1).at(k).velocityZ) / dx +
                                    ...
                                    mesh.at(i).at(j).at(k).velocityZ * (mesh.at(i).at(j).at(k).velocityZ - mesh.at(i - 1).at(j).at(k).velocityZ) / dz) /
                                  dz;
        ...
        for (int j = 1; j < nx - 1; j += (nx / (nx / 2)))
        {
            for (int k = 1; k < ny - 1; k += (ny / (ny / 4)))
            {
                ...
                else
                {
                    ...
                    DrawLine3D(point, velocityDirection, velocityColor);
                }
            }
        }
    }
...
void Cfd::run(int steps, double stepsizePitch, double stepsizeYaw)
{
    ...
    { // pitch
        ...
        for (double j = 0; j <= 360; j += stepsize)
        { // yaw
